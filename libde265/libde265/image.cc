...
#else
#define  ALLOC_ALIGNED(data,alignment, size)  posix_memalign((data),(alignment), (size))
//#define ALLOC_ALIGNED(data,alignment, size)      memalign((alignment), (size))
#define FREE_ALIGNED(mem)                   free((mem))
#endif

#define ALLOC_ALIGNED_16(data,size)              ALLOC_ALIGNED((data),16,(size))
                                           ...
                                           void* inputdata, int inputstride, void *userdata)
{
  ...
  int height = img->get_height(cIdx);
    ...
    uint8_t* p;
    ALLOC_ALIGNED_16((void**)&p,stride * height + MEMORY_PADDING);
                                   ...
                                   de265_image_spec* spec, de265_image* img, void* userdata)
{
  ...
  uint8_t* p[3] = { 0,0,0 };
  ALLOC_ALIGNED_16((void**)&p,luma_height   * luma_bpl   + MEMORY_PADDING);
  if (p[0]==NULL) { alloc_failed=true; }

  if (img->get_chroma_format() != de265_chroma_mono) {
    ALLOC_ALIGNED_16((void**)&(p[1]),chroma_height * chroma_bpl + MEMORY_PADDING);
    ALLOC_ALIGNED_16((void**)&(p[2]),chroma_height * chroma_bpl + MEMORY_PADDING);
